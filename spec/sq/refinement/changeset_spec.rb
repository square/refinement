...
RSpec.describe Sq::Refinement::Changeset do
  ...
  def mod(path, type: :changed, prior_path: nil, contents: nil, prior_contents: nil)
    described_class::FileModification.new(path: Pathname(path), type:, prior_path: prior_path && Pathname(prior_path),
  ...
  describe '.parse_raw_diff' do
    subject(:modifications) { described_class.parse_raw_diff(diff, repository: repo, base_revision:) }
